<?php
/* Archivo: ajax/clientes/modificar.php */
...
try {
    ...
    if ($id <= 0) {
        throw new Exception('El identificador del cliente no es válido.');
    }
    ...
    $clienteExiste = $clsConsulta->consultaPreparada(
        "SELECT id FROM cat_clientes WHERE id = ? AND deleted_at IS NULL LIMIT 1",
        [$id]
    );
    ...
    $clsConsulta->actualizarSeguro('cat_clientes', $id, $datos);
    ...
    error_log('Error en ajax/clientes/modificar.php: ' . $e->getMessage());
